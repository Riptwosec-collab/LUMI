...
const DEFAULT_PARAMS = Object.freeze({
  ...
  levelsBlackOut: 0,
  levelsWhiteOut: 1,
  balanceShadowWarmth: 0,
  balanceShadowTint: 0,
  balanceMidWarmth: 0,
  balanceMidTint: 0,
  balanceHighWarmth: 0,
  balanceHighTint: 0,
  mixRR: 1, mixRG: 0, mixRB: 0,
  mixGR: 0, mixGG: 1, mixGB: 0,
  mixBR: 0, mixBG: 0, mixBB: 1,
  replaceHue: 0,
  replaceRange: .12,
  replaceShift: 0,
  replaceSat: 0,
...
const SLIDERS = {
  ...
  levels: [
    ['levelsBlackIn','Black Input',0,.45,.005,v=>Math.round(v*255)],
    ['levelsGamma','Midtone Gamma',.25,3,.01,v=>v.toFixed(2)],
    ['levelsWhiteIn','White Input',.55,1,.005,v=>Math.round(v*255)],
    ['levelsBlackOut','Black Output',0,.4,.005,v=>Math.round(v*255)],
    ['levelsWhiteOut','White Output',.6,1,.005,v=>Math.round(v*255)]
  ],
  grading: [
    ['balanceShadowWarmth','Shadow Warmth',-1,1,.01,pct],['balanceShadowTint','Shadow Tint',-1,1,.01,pct],
    ['balanceMidWarmth','Midtone Warmth',-1,1,.01,pct],['balanceMidTint','Midtone Tint',-1,1,.01,pct],
    ['balanceHighWarmth','Highlight Warmth',-1,1,.01,pct],['balanceHighTint','Highlight Tint',-1,1,.01,pct]
  ],
  tonal: [
    ['gamma','Gamma',.45,2.2,.01,v=>v.toFixed(2)],['tonalShadows','Shadow Contrast',-1,1,.01,pct],
    ['tonalMidtones','Midtone Contrast',-1,1,.01,pct],['tonalHighlights','Highlight Contrast',-1,1,.01,pct]
  ],
  mixer: [
    ['mixRR','Red ← Red',-1,2,.01,v=>v.toFixed(2)],['mixRG','Red ← Green',-1,2,.01,v=>v.toFixed(2)],['mixRB','Red ← Blue',-1,2,.01,v=>v.toFixed(2)],
    ['mixGR','Green ← Red',-1,2,.01,v=>v.toFixed(2)],['mixGG','Green ← Green',-1,2,.01,v=>v.toFixed(2)],['mixGB','Green ← Blue',-1,2,.01,v=>v.toFixed(2)],
    ['mixBR','Blue ← Red',-1,2,.01,v=>v.toFixed(2)],['mixBG','Blue ← Green',-1,2,.01,v=>v.toFixed(2)],['mixBB','Blue ← Blue',-1,2,.01,v=>v.toFixed(2)]
  ],
  replacecolor: [
    ['replaceHue','Target Hue',0,1,.005,v=>`${Math.round(v*360)}°`],['replaceRange','Range',.02,.4,.005,v=>`${Math.round(v*100)}%`],
    ['replaceShift','Hue Shift',-1,1,.01,pct],['replaceSat','Saturation',-1,1,.01,pct],['replaceLum','Luminance',-1,1,.01,pct]
  ],
  optics: [
    ['lensVignette','Lens Vignette',-1,1,.01,pct],['chromaticAberration','Chromatic Fringe',0,1,.01,pct]
  ],
  film: [
    ['filmFade','Fade',0,1,.01,pct],['halation','Halation',0,1,.01,pct],['grain','Grain',0,1,.01,pct],['bloom','Bloom',0,1,.01,pct],['vignette','Vignette',0,1,.01,pct]
  ],
  blur: [
    ['lensBlur','Lens Blur',0,1,.01,pct],['bloom','Highlight Boost',0,1,.01,pct],['vignette','Edge Focus',0,1,.01,pct]
  ]
};
...
const INFO_FEATURES = {
  raw: {
    ...
    bullets: ['Native iOS จะใช้ Core Image RAW pipeline', 'PWA ไม่ hard-code camera support', 'JPEG/PNG/WebP workflowทำงานเต็มกว่า']
  },
  hdr: {
    title: 'HDR Editing',
    status: 'ROADMAP',
